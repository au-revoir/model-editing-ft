...
from single_eval_losses import CustomDPOTrainer
from data_dpo import DPODataset
...
def get_dataset(path, tokenizer):
    ...
    for i in data:
        ...
        combined_prompts = original_rewrite_prompt #+ neighborhood_prompts #+ paraphrase_prompts #+ neighborhood_prompts
        combined_chosen =  original_rewrite_chosen #+ chosen_neighborhood_answers #+ chosen_paraphrase_answers#+ chosen_neighborhood_answers
        combined_rejected = original_rewrite_rejected #+ rejected_neighborhood_answers #+ rejected_paraphrase_answers #+ rejected_neighborhood_answers
        #requested_rewrite = i["requested_rewrite"]
        #subject = requested_rewrite["subject"]
...
def main(script_args):
    ...
    #print(state_dict)
    model = AutoModelForCausalLM.from_pretrained("gpt2-xl", torch_dtype=torch.float16, #device_map={'':torch.cuda.current_device()}, 
                                                            device_map={"": Accelerator().local_process_index}, 
                                                            #device_map={'':torch.cuda.current_device()},
                                                            cache_dir="/data/local/gg676/pretrained_models")

    model_reference = AutoModelForCausalLM.from_pretrained("gpt2-xl", torch_dtype=torch.float16, #device_map={'':torch.cuda.current_device()}, 
                                                            device_map={"": Accelerator().local_process_index}, 
                                                            cache_dir="/data/local/gg676/pretrained_models")
    """
    ...
    #for i in range(train_edit_data):
    ...
    dpo_trainer = CustomDPOTrainer(#DPOTrainer(
                            ...
                            state_dict=model_reference.state_dict()
    ...
    dpo_trainer.train()
    quit()
    #for i in range(len(train_edit_data)):
    #    print("i: ", i)
    #    dpo_trainer.train(example_idx=i)
    globals.eval_method.final_evaluation_scores()
    ...
    dpo_trainer = CustomDPOTrainer(#DPOTrainer(
                            ...
                            args=training_args,
                            beta=script_args.beta,
                            train_dataset=train_edit_data,#dataset,
                            #eval_dataset=eval_dataset,
                            tokenizer=tokenizer,
                            #peft_config=peft_config,
                            eval_edit_data=eval_edit_data,
    ...
    for item, item_original in tqdm(data[script_args.data_size:script_args.data_size+10]):
    #for item, item_original in tqdm(data[:1]):
        #set_generate_seed()
        #from trl import DPOTrainer
        ...
        with torch.no_grad():
            for name, param in model_reference.named_parameters():
                w = get_parameter(model_reference, name)
                for n, p in model.named_parameters():
                    if name == n:
                        p[...] = w.detach().to("cuda")
        """
        #prompt = item["prompt"][0] + item["chosen"][0]
        #print("prompt: ", prompt)
        #tokenized_inputs = tokenizer(prompt, padding="max_length", max_length=150, return_tensors="pt")
