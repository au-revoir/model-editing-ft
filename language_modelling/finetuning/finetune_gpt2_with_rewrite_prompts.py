...
from transformers import AutoModelForCausalLM, AutoTokenizer, HfArgumentParser, TrainingArguments, GPT2LMHeadModel
from transformers import Trainer, set_seed, DataCollatorWithPadding, DataCollatorForLanguageModeling
...
def get_dataset_1(path, tokenizer):
    with open(path) as fp:
        data = json.load(fp)
    data_modified = []
    #print(data[0])
    ...
    for i in data:
        ...
        original_rewrite_question = requested_rewrite["prompt"].format(subject)
        original_target_new = " " + requested_rewrite["target_new"]["str"]
        original_target_true = " " + requested_rewrite["target_true"]["str"]

        original_prompt = [original_rewrite_question + original_target_new]
        
        """
        paraphrase_questions = i["paraphrase_prompts"]
        paraphrase_prompts = []
        for paraphrase_question in paraphrase_questions:
            paraphrase_prompts.append(paraphrase_question + original_target_new)

        neighborhood_questions = i["neighborhood_prompts"]

        neighborhood_prompts = []
        for neighborhood_question in neighborhood_questions:
            neighborhood_prompts.append(neighborhood_question + original_target_true)
        """
        #sample = original_prompt + paraphrase_prompts + neighborhood_prompts
        sample = original_prompt
        #print(item)
        tokenized_inputs = tokenizer(sample, padding="longest", return_tensors="pt")
        ...
        """
        #neighborhood_prompts = [neighborhood_prompt + " " for neighborhood_prompt in i["neighborhood_prompts"]]
        """
        ...
        #data_modified.append(sample)
        data_modified.append((tokenized_inputs, i))
...
def initialize_training_args(script_args, max_steps):
    training_args = TrainingArguments(
        per_device_train_batch_size=script_args.per_device_train_batch_size,
        #per_device_eval_batch_size=script_args.per_device_eval_batch_size,
        max_steps=max_steps,#script_args.max_steps,
...
def perform_evaluation(model, data):
    
    prompt_total_count = 0
    prompt_magnitude = 0.0
    for item in data:
        ...
        ppl_rewrite_prompt_target_new, ppl_rewrite_prompt_target_true = evaluate_model(trainer.model, tokenizer, rewrite_prompt, targets)
    ...
    print("rewrite_success_accuracy: ", rewrite_success_count / len(data))
    print("rewrite_magnitude: ", rewrite_magnitude / len(data))
...
def main(script_args):
    ...
    """
    model_base = AutoModelForCausalLM.from_pretrained("gpt2-xl", torch_dtype=torch.bfloat16, #device_map={'':torch.cuda.current_device()}, 
                                                            #device_map={"": Accelerator().local_process_index}, 
    ...
    #print(state_dict)
    model = GPT2LMHeadModel.from_pretrained("gpt2-xl", torch_dtype=torch.bfloat16, #device_map={'':torch.cuda.current_device()}, 
                                                            device_map={"": Accelerator().local_process_index}, 
                                                            #device_map={'':torch.cuda.current_device()},
                                                            cache_dir="/data/local/gg676/pretrained_models")
    ...
    """
    model_reference = AutoModelForCausalLM.from_pretrained("gpt2-xl", torch_dtype=torch.bfloat16, #device_map={'':torch.cuda.current_device()}, 
                                                            #device_map={"": Accelerator().local_process_index}, 
                                                            cache_dir="/data/local/gg676/pretrained_models")
    """
    """
    ...
    data = get_data(script_args.data_path)[:script_args.data_size] #Use only 2000 examples for editing
    #print(data[0])
    #quit()
    dataset = FinetuneDataset(data, script_args.rewrite_prompt, script_args.paraphrase_prompt, script_args.neighborhood_prompt , tokenizer)
    print("len(dataset): ", len(dataset))
    #print("dataset[0]: ", dataset[0])
    #quit()
    #dataset = FinetuneDataset(data, tokenizer)
    #print("data: ", data[0])
    #quit()
    ...
    training_args = initialize_training_args(script_args, max_steps=script_args.step_multiplier*len(dataset))
    ...
    for item in data:
        ...
        for paraphrase in item["paraphrase_prompts"]:
            ppl_paraphrase_target_new, ppl_paraphrase_target_true = evaluate_model(trainer.model, tokenizer, paraphrase, targets)
        ...
        for neighbor in item["neighborhood_prompts"]:
            ppl_neighborhood_target_new, ppl_neighborhood_target_true = evaluate_model(trainer.model, tokenizer, neighbor, targets)
    ...
    for item, item_original in tqdm(data[:30]):
        #from trl import DPOTrainer
        ...
        #del state_dict
        #print("Before edit")
        #compare_models(model, model_reference.to("cuda"))
        ...
        #tokenized_inputs = tokenizer(item, padding=True, max_length=120, return_tensors="pt").to("cuda")
        #print("item: ", item)
        #print("item_original: ", item_original)
        #quit()
        #dataset = Dataset.from_list(item)
