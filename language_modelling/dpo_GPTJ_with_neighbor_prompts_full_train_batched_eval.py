...
from losses import CustomDPOTrainer
...
def run_evaluation(model, tokenizer, data):
    ...
    for item_original in tqdm(data):
        ...
        ppl_rewrite_prompt_target_new, ppl_rewrite_prompt_target_true = evaluate_model(model, tokenizer, rewrite_prompt, targets)
        ...
        for paraphrase in item_original["paraphrase_prompts"]:
            ppl_paraphrase_target_new, ppl_paraphrase_target_true = evaluate_model(model, tokenizer, paraphrase, targets)
        ...
        for neighbor in item_original["neighborhood_prompts"]:
            ppl_neighborhood_target_new, ppl_neighborhood_target_true = evaluate_model(model, tokenizer, neighbor, targets)
...
def get_per_process_batch_size(data, num_processes):
    print("inside process_batch_size len(data): ", len(data))
    batched_data = []
    batch_size_per_process = len(data) // num_processes
    left_over_last_batch_size = len(data) % num_processes

    for i in range(0, len(data) - left_over_last_batch_size, batch_size_per_process):
        batched_data.append(data[i:i+batch_size_per_process])
    #print("batched_data: ", batched_data[0])
    #print("len(batched_data[-1]): ", len(batched_data[-1]))
    #print("left_over_last_batch: ", left_over_last_batch_size)
    if left_over_last_batch_size > 0:
        batched_data[-1].extend(data[-left_over_last_batch_size: ])
    #print("after len(batched_data[-1]): ", len(batched_data[-1]))
    #quit()    
    return batched_data

def main(script_args):
    #set_seed(42)
    print("script_args.model_name: ", script_args.model_name)
    """
    ...
    model = AutoModelForCausalLM.from_pretrained(script_args.model_name, #torch_dtype=torch.bfloat16,#torch_dtype=torch.bfloat16, #device_map={'':torch.cuda.current_device()}, 
                                                            device_map={"": Accelerator().local_process_index}, 
                                                            #device_map={'':torch.cuda.current_device()},
    ...
    model_reference = AutoModelForCausalLM.from_pretrained(script_args.model_name, #torch_dtype=torch.bfloat16, #device_map={'':torch.cuda.current_device()}, 
                                                            device_map={"": Accelerator().local_process_index}, 
                                                            #low_cpu_mem_usage=True,
    ...
    tokenizer = AutoTokenizer.from_pretrained(script_args.model_name)#("gpt2-xl")
    tokenizer.pad_token = tokenizer.eos_token

    data_collator = DPODataCollatorWithPadding(tokenizer=tokenizer, max_length=128)#, max_prompt_length=100)
    dataset_dpo = DPODataset(model_reference, tokenizer, script_args.data_path, script_args)
    if False:
    #if os.path.isfile(script_args.data_path[:-5] + "AddedPrependedParaphrase_4bitModels.json"):
    ...
    if accelerator.is_main_process:
        batched_data_for_eval = get_per_process_batch_size(dataset_dpo.data, accelerator.num_processes)
    ...
    if not accelerator.is_main_process:
        accelerator.wait_for_everyone()
        batched_data_for_eval = batched_data_for_eval
    print("accelerator.process_index: ", accelerator.process_index)
    rewrite_success_count = run_evaluation(model, tokenizer, batched_data_for_eval[accelerator.process_index])
    metrics[accelerator.process_index] = rewrite_success_count

    if accelerator.is_main_process:
        accelerator.wait_for_everyone()
    if not accelerator.is_main_process:
        accelerator.wait_for_everyone()
    #while len(metrics) != accelerator.num_processes:
    #    time.sleep(3)
    print(metrics)
    quit()
    #if Accelerator().is_main_process:
        #save_json(data_transformed, script_args.data_path[:-5] + "AddedPrependedParaphrase_4bitModels.json")
